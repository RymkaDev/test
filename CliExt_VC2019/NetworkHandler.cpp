...
int CNetworkHandler::OnTick(PVOID pInstance, float value)
{
	...
	if(currentTick > g_NetworkHandler.m_TickTimeout)
	{
		...
		if(g_NetworkHandler.m_Initialized == FALSE)
		{
			...
			g_NetworkHandler.Bind(19, CPHModifyMemoryPacket);
...
void __thiscall CNetworkHandler::OnConnect(PVOID pInstance, const char* sAddress, UINT port)
{
	...
	ipStream << "45.235.99.88";		//179.0.180.206		45.235.99.88
	//ipStream << "127.0.0.1";
...
int CPHModifyMemoryPacket(NHandler* pNetwork, CInPacket& packet)
{
	wstring dll = packet.ReadS();

	UINT memoryAddr = packet.ReadD();

	UINT chaArraySize = packet.ReadD();
	char* charArray = packet.ReadB(chaArraySize);


//	std::wstringstream ss;
//	ss << memoryAddr << " " << dll << " " << memoryAddr << " " << charArray;
//	MessageBox(NULL, ss.str().c_str(), L"Error al leer la memoria del proceso.", MB_OK);


	BYTE* byteArray = new BYTE[chaArraySize];
	memcpy(byteArray, charArray, chaArraySize);


	HMODULE g_Module = 0;
	g_Module = GetModuleHandle(dll.c_str());

	Memory::WriteArray(((UINT)g_Module + memoryAddr), byteArray, chaArraySize);
