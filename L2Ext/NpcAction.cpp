...
void NpcAction::EnterWorld(CNPC* pNPC, bool bSetDefaultParam, int nHP, int nMP)
{
	if(pNPC->ValidCreature())
	{
		...
		/*if(g_ChampionNpc.IsEnabled())
		{
			if(g_ChampionNpc.IsChampionNPC(npcId))
			{
				ChampionNpcInfo* pInfo = g_ChampionNpc.GetChampionData(npcId);
				double dice = g_Random.RandDouble(100.0);
				if( dice < pInfo->chance_red )
				{
					pNPC->pSD->teamType = 2; //Red
					//Change HP/MP
					int m_OldHP = nHP;
					int m_OldMP = nMP;
					int m_NewHP = ((m_OldHP/100)*g_ChampionNpc.GetRedHPMPAddon()) + nHP;
					int m_NewMP = ((m_OldMP/100)*g_ChampionNpc.GetRedHPMPAddon()) + nMP;
					nHP = m_NewHP;
					nMP = m_NewMP;
					
				}
				else if( dice < pInfo->chance_blue )
				{
					pNPC->pSD->teamType = 1; //Blue
					//Change HP/MP
					int m_OldHP = nHP;
					int m_OldMP = nMP;
					int m_NewHP = ((m_OldHP/100)*g_ChampionNpc.GetBlueHPMPAddon()) + nHP;
					int m_NewMP = ((m_OldMP/100)*g_ChampionNpc.GetBlueHPMPAddon()) + nMP;
					nHP = m_NewHP;
					nMP = m_NewMP;
				}
			}
