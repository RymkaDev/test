						...
						if(nEnchantLevel >= cad.minEnchant)
						{
							...
							}else
							{
								...
								if(cad.isBlessed)
								{
									...
									if(g_StackableEnchantScrolls)
									{
										g_DB.RequestCustomEnchantItem(pUser->nDBID, pItem->nDBID, cad.safeEnchantLevel, pScroll->pSID->nItemID);
									}else
									{
										g_DB.RequestEnchantItem(pScroll, pItem, cad.safeEnchantLevel, pUser);
									}
								}else
								{
									if(nEnchantLevel)
