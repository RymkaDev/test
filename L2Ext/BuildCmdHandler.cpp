...
void CBuilderCommand::Initialize()
{
	...
	Info.Set(L"jail", 3, CBuilderCommand::JailPlayer);	mData.insert(pair<wstring, CCommandInfo>(Info.wsName, Info));
...
bool CBuilderCommand::TestCMD(User *pUser, std::wstring wsParams)
{
	if(pUser->ValidUser())
	{
		...
		{
			//(UINT64)this + 0x26B8)
//			int nPackageSell = *(DWORD*)pTarget + 0x279C;

//			g_Log.Add(CLog::Blue, "Package Selling Debug1[%d] Debug2[%d] Debug3[%d]", pTarget->pSD->m_nPrivateStoreLimit, pTarget->pSD->m_nPrivateStoreBuyLimit, pTarget->pSD->m_nPrivateStoreRecipeType);
		}
	}
	return false;
}
